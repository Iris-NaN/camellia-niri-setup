...
colors_shell="$HOME/.cache/wal/colors.sh"

if [[ -r "$colors_shell" ]]; then
    # Keep Pywal as the source of truth; consume its generated shell palette.
    source "$colors_shell"
else
    echo "Pywal colors were not generated: $colors_shell" >&2
    exit 1
fi
...
# Reload SwayNC after updating its generated color stylesheet.
"$scripts_dir/sync-pywal-targets.sh"
